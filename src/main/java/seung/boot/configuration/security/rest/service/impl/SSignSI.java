package seung.boot.configuration.security.rest.service.impl;
...
import seung.boot.configuration.datasource.SMapper0;
import seung.boot.configuration.security.crypto.SBCryptPasswordEncoder;
import seung.boot.configuration.security.crypto.SJwt;
import seung.boot.configuration.security.rest.service.SSignS;
...
import seung.boot.configuration.security.types.SCommonOAuth2Provider;
import seung.boot.configuration.security.types.SIdTokenClaimNames;
import seung.boot.configuration.security.types.SKey;
import seung.boot.configuration.security.types.SOAuth2AuthenticationException;
import seung.boot.configuration.security.types.SUser;
import seung.boot.configuration.web.types.SError;
import seung.boot.configuration.web.types.SRequestAttribute;
import seung.boot.configuration.web.types.SResponseBody;
import seung.boot.configuration.web.types.SResponseEntity;
import seung.boot.configuration.web.types.SResponseException;
...
public class SSignSI implements SSignS {
	...
	public ResponseEntity<SResponseBody> signin_username(
		...
		try {
			...
			throw new SResponseException(e, response_body.done(SError.SIGNIN_FAILED));
		} catch (DisabledException e) {
			log.error("({}) exception=", trace_id, e);
			// 계정잠김
			throw new SResponseException(e, response_body.done(SError.SIGNIN_DISABLED));
			...
			log.error("({}) exception=", trace_id, e);
			// 비밀번호만료
			throw new SResponseException(e, response_body.done(SError.PASSWORD_EXPIERED));
		} catch (LockedException e) {
			log.error("({}) exception=", trace_id, e);
			// 인증 5회 실패
			throw new SResponseException(e, response_body.done(SError.SIGNIN_LOCKED));
		} catch (Exception e) {
			log.error("({}) exception=", trace_id, e);
			throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR);
		} finally {
